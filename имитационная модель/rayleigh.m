...
switch chanType
    case 'eva'
        ...
        chan.Seed = 42;
    case 'simple'
        chan.PDelaysNs = [0, 30, 150, 300, 350, 700, 1000, 1500, 2500, 5400] * 1e-9;
...
for SNR = 1:length(EbN0)
    for Fr=1:numFrames % цикл по кадрам
        for mes = 1:2 % цикл по сообщениям, где 1 - преамбула, 2 - информацинное сообщение
            if mes == 1
                preambule_ofdm = cell(length(EbN0), 2);
                preambule_ocdm = cell(length(EbN0), 2);
            end
        %=============Transmitter==============
        ...
        %         tmp3 = [txPreambule_ofdm(end - cpLen+1:end); txPreambule_ofdm];
            
                txSignal_ocdm = tmp1;
                txSignal_ofdm = tmp2;
        %         txPreambule_ext = tmp3;
    ...
    %         for SNR = 1:length(EbN0)
        ...
        %             rxPreambule = rxPreambule(cpLen+1:end);
                ...
                if mes == 1
                    preambule_ocdm{SNR, 1} = rxSignal_ocdm;
                    preambule_ocdm{SNR, 2} = preambule_ocdm{1, 2};
                    preambule_ofdm{SNR, 1} = rxSignal_ofdm;
                    preambule_ofdm{SNR, 2} = preambule_ofdm{1, 2};
                    continue
                end
        ...
        %-------Эквализация------        
                ...
                preambule_ocdm{SNR, 1} = fft(preambule_ocdm{SNR, 1}, N);
                preambule_ocdm{SNR, 2} = fft(preambule_ocdm{SNR, 2}, N);
                preambule_ocdm{SNR, 1} = preambule_ocdm{SNR, 1}.*phaseRotate;
                preambule_ocdm{SNR, 2} = preambule_ocdm{SNR, 2}.*phaseRotate;
                chanEst_ocdm = (preambule_ocdm{SNR, 1}./preambule_ocdm{SNR, 2});
    ...
    %             rxSignal_ocdm_rot = rxSignal_ocdm_fft;
                switch eqType
                    case 'zf'
                        rxSignal_ocdm_eq = rxSignal_ocdm_rot./chanEst_ocdm;
                    case 'mmse'
    ...
    %     %-------OFDM-------
        ...
        %-------Эквализация-
                preambule_ofdm{SNR, 1} = (1/sqrt(N))*fftshift(fft(preambule_ofdm{SNR, 1}, N));
                chanEst_ofdm = preambule_ofdm{SNR, 1}./preambule_ofdm{SNR, 2};
    %             stem(10*log10(fftshift(angle(chanEst_ofdm))));
    %             hold off;
                switch eqType
                    case 'zf'
...
%                 figure(2);plot(rxSyms_ocdm, 'o'); xlim([-2 2]), ylim([-2 2]); pause(0.1)
    ...
    clear preambule_ofdm preambule_ocdm
end
toc;
